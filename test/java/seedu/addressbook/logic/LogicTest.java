...
public class LogicTest {
    ...
    public void execute_add_invalidPersonData() throws Exception {
        assertCommandBehavior(
                "add []\\[;] p/12345 e/valid@e.mail a/valid, address, m/30November s/observation", Name.MESSAGE_NAME_CONSTRAINTS);
        assertCommandBehavior(
                "add Valid Name p/not_numbers e/valid@e.mail a/valid, address, m/30November s/observation", Phone.MESSAGE_PHONE_CONSTRAINTS);
        assertCommandBehavior(
                "add Valid Name p/12345 e/notAnEmail a/valid, address, m/30November s/observation", Email.MESSAGE_EMAIL_CONSTRAINTS);
        assertCommandBehavior(
                "add Valid Name p/12345 e/valid@e.mail a/valid, address, m/30November, s/observation, t/invalid_-[.tag", Tag.MESSAGE_TAG_CONSTRAINTS);
    ...
    class TestDataHelper{
        ...
        Person adam() throws Exception {
            ...
            return new Person(name, privatePhone, email, privateAddress, appointment, status, tags);
        }
        ...
        String generateAddCommand(Person p) {
            ...
            cmd.add((p.getAddress().isPrivate() ? "pa/" : "a/") + p.getAddress() + "m/"+p.getAppointment() + "s/"+p.getStatus());
