...
public class LogicTest {
    ...
    @Test
    public void execute_sortByEmail() throws Exception {
        // prepare expectations
        TestDataHelper helper = new TestDataHelper();
        AddressBook expectedAB = helper.generateAddressBook(false, true);
        expectedAB.sortByEmail();
        List<? extends ReadOnlyPerson> expectedList = expectedAB.getAllPersons().immutableListView();

        // prepare address book state
        helper.addToAddressBook(addressBook, false, true);

        assertCommandBehavior("sortByEmail",
                Command.getMessageForPersonSortByEmailShownSummary(expectedList),
                expectedAB,
                true,
                expectedList);
    }
