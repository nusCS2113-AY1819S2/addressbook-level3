...
== Finding all persons with keywords in their tags: `findtag`

Finds persons who have such tags +
Format: `findtag KEYWORD [MORE_KEYWORDS]`

[NOTE]
====
The search is case sensitive, the order of the keywords does not matter, only the tags is searched,
and persons matching at least one keyword will be returned (i.e. `OR` search).
====
...
* `findtag friend` +
Returns `John Doe` with the tag 'friend'.

* `find friend myself` +
Returns Any person having tags `friend` or `myself`.
