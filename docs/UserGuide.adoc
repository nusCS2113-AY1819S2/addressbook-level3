...
//@@author shawn-t
== Referring patient indicated by user : `refer`
Refers a patient from the address book to a doctor. +
Refer is overloaded with two formats.

Format1: `refer NAME [MORE_NAMES]`
Refers target patient to default doctor, Dr Seuss, and shows a success message with patient's updated particulars if there is only one unique entry. +
if there are multiple entries, all patient entries with the target keywords in the address book will be printed out and user will be prompted to identify the correct patient. +
if patient does not exist in the addressbook, prints out an error message. +
...
Refers target patient to a doctor specified by user. Format 2 works exactly like format 1. +
If input contains an invalid doctor names (non-alpha numerical), an invalid command format message will be shown.
