...
. AddressBook finds patient from the list of entries, and extract all the relevant particulars of the target patient.
. Addressbook stores the extracted particulars in a temporary location and updates it with the new doctor's name and patient tag.
. AddressBook deletes the old patient entry from the main list and updates the main list with the newly updated entry.
+
Use case ends.
